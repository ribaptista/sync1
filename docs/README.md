...
- **Password scope**: only commands that read or write encrypted content (state.db snapshots or content objects) prompt for the vault password — `init_remote`, `attach_remote`, `fetch_remote`, `sync`, `gc`, `materialize`, `ignore create`/`edit`/`delete`, and `storage_policy create`/`edit`/`delete` (each of those last two is a real state.db commit). `update_cache`, `inspect`, `stubify`, `ensure_storage_class`, `ignore list`, `sanity_check`, `storage_policy list`, and `status` never need it: the `list`/`inspect`/`sanity_check`/`update_cache`/`status` group only ever read the unencrypted `cache.db` or the already-locally-decrypted `state.db`, `stubify` only ever removes local files it already has and writes a stub containing a hash already recorded locally, and `ensure_storage_class`/`sanity_check`/`status` only touch S3 storage-class metadata/existence (`HEAD`) without reading or writing object content.
